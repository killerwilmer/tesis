...
<html>
    <head>
        ...
        <title>Listado de correciones</title>
    </head>
    <body>
    <center>
        <br>
        ...
        <table cellpadding="0" cellspacing="0" border="0" class="display" id="example" width="90%" style="text-align: center">
            <thead >
                <tr><td class="columna1">No.</td><td class="columna2">Fecha</td><td class="columna3">Correccion</td></tr>
            </thead>
            <tbody>
                <%
                    String sql = "select fechacorreccion, temacorreccion from correccion where idproyecto=" + idProyecto + " and idusuario=" + idDocente + ";";
                    ResultSet datos = control.consultas(sql);

                    while (datos.next()) {

                        out.print(control.linea(i) + "<td name='cod'>" + (i + 1) + "</td>"
                                + "<td name='idproyec'>" + datos.getString("fechacorreccion") + "</td>"
                                + "<td name='titulop'>" + datos.getString("temacorreccion") + "</td>");
                        i++;
                    }
